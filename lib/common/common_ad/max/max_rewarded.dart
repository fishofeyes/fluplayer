...
class MaxRewardLoader extends BaseAd {
  String? adId;
  ...
  Future<void> loadAD(
    ...
    AppLovinMAX.loadRewardedAd(adPlacement);
  }

  @override
  Future<void> show({CommAdShowListener? listener}) async {
    if (adId == null) {
      listener?.error?.call(CommonAdLoadError('-1', 'max ad is null'));
      return;
    }
    maxHelper.addShowListener(
      ...
      onShow: CommAdShowListener(
        ...
        onReward: (result) {
          listener?.onReward?.call(result);
        },
      ),
    );
    final isOk = await AppLovinMAX.isRewardedAdReady(adId!) ?? false;
    if (isOk) {
