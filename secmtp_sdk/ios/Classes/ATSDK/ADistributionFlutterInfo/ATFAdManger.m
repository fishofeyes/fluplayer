...
@property(nonatomic, strong) NSArray *funcSplashNameArray;
...
- (void)distributionFlutterInformation:(FlutterMethodCall*)call result:(FlutterResult)result{
    ...
    // 开屏广告调用
    if ([self.funcSplashNameArray containsObject:call.method]) {
        handled = YES;
        [self splashAdFlutterInformation:call result:result];
    }
...
- (NSArray *)funcinitNameArray{
    if (_funcinitNameArray == nil) {
        _funcinitNameArray = @[SetLogEnabled,SetChannelStr,SetSubchannelStr,SetCustomDataDic,SetExludeBundleIDArray,SetPlacementCustomData,InitSDK,SetPresetPlacementConfigPath,GetGDPRLevel,GetUserLocation,ShowGDPRConsentDialog,ShowGDPRConsentSecondDialog,CheckIsEuTraffic,ShowGDPRAuth,ShowDebugUI,SetDataConsentSet,SetDeniedUploadInfoArray,SetAdSourcePrivacyPolicy,PutFilter,RemoveFilters,RemoveFilterWithPlacementId,GetSDKVersionName,SetSharedPlacementConfig,StartSDK];
    }
    return _funcinitNameArray;
}

- (NSArray *)funcSplashNameArray{
    if (_funcSplashNameArray == nil) {
        _funcSplashNameArray = @[LoadSplashAd,SplashAdReadyReady,CheckSplashAdLoadStatus,ShowSplashAd,ShowSceneSplashAd,GetSplashAdValidAds,ShowSplashAdWithShowConfig,EntrySplashScenario];
    }
    return _funcSplashNameArray;
}

- (NSArray *)funcRewardNameArray{
    if (_funcRewardNameArray == nil) {
        _funcRewardNameArray = @[LoadRewardedVideo,RewardedVideoReady,CheckRewardedVideoLoadStatus,ShowRewardedVideo,GetRewardedVideoValidAds,ShowSceneRewardedVideo,ShowRewardedVideoWithShowConfig,EntryRewardedVideoScenario,AutoLoadRewardedVideo,CancelAutoLoadRewardedVideo,ShowAutoLoadRewardedVideoAD,AutoLoadRewardedVideoSetLocalExtra];
    }
...
- (NSArray *)funcInterstitialNameArray{
    if (_funcInterstitialNameArray == nil) {
        _funcInterstitialNameArray = @[LoadInterstitialAd,HasInterstitialAdReady,GetInterstitialValidAds,ShowInterstitialAd,CheckInterstitialLoadStatus,ShowSceneInterstitialAd,ShowInterstitialAdWithShowConfig,EntryInterstitialScenario,AutoLoadInterstitialAD,CancelAutoLoadInterstitialAD,ShowAutoLoadInterstitialADWithPlacementID,AutoLoadInterstitialADSetLocalExtra];
    }
...
- (NSArray *)funcBannerNameArray{
    if (_funcBannerNameArray == nil) {
        _funcBannerNameArray = @[LoadBannerAd,BannerAdReady,CheckBannerLoadStatus,GetBannerValidAds,ShowBannerInRectangle,ShowAdInPosition,RemoveBannerAd,HideBannerAd,AfreshShowBannerAd,ShowSceneBannerAdInPosition,ShowSceneBannerInRectangle,EntryBannerScenario];
    }
...
- (NSArray *)funcNativeNameArray{
    if (_funcNativeNameArray == nil) {
        _funcNativeNameArray = @[NativeAdReady,CheckNativeAdLoadStatus,GetNativeValidAds,LoadNativeAd,ShowNativeAd,ShowSceneNativeAd,RemoveNativeAd,ShowSceneNativeAdWithCustomExt,EntryNativeScenario];
    }
...
- (ATFSplashAdManger *)splashAdManger {

    if (_splashAdManger) return _splashAdManger;

    ATFSplashAdManger *splashAdManger = [ATFSplashAdManger new];

    return _splashAdManger = splashAdManger;
}
@end
